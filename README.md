...
For any matrix $S \in \mathbb{R}^{n \times m}$, the matrix $P = S S^T$ is positive semidefinite (PSD). This esoteric package implements a positive semidefinite matrix type by storing the square root matrix $S$ instead of the full matrix $P$. [PDMats.jl](https://github.com/JuliaStats/PDMats.jl) can't handle the semidefinite case (rightly so) and [PDMatsExtra.jl](https://github.com/invenia/PDMatsExtras.jl) lacks some of the operations I want, so I am creating my own small version. 
